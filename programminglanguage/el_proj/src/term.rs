...
impl Term {
    ...
    pub fn is_value(&self) -> bool {
        match self {
            ...
            True | False => true,
            Int(_) => true,
            Pair(a, b) => a.as_ref().is_value() && b.as_ref().is_value(),
            Nil(T) => true,
            Cons(h, t) => h.as_ref().is_value() && t.as_ref().is_value(),
            Inl(ter, typ) => ter.as_ref().is_value(),
            Inr(ter, typ) => ter.as_ref().is_value(),
            _ => false,
        }
    }
}
