...
/*
fn parse_named_fn(input: &str) -> IResult<&str, Term> {
    (tag("function"), 
    multispace1, 
    parse_variable_name, 
    multispace0, 
    ...
    multispace0, 
    parse_block)
        .map(|(_, _, name, _, param, _, body)| App(Box::new(Abs {}), Box::new())
        )
        .parse(input)
...
/*
fn parse_print(input: &str) -> IResult<&str, Term> {
    (tag("print"), multispace1, parse_term)
        .map(|(_, _, term)|  )
}
*/
