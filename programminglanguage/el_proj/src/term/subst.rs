    ...
    pub fn subst(self, x: &str, v: Self) -> Self {
        match self {
            ...
            Ite {
                ...
                if_false: Box::new(if_false.subst(x, v.clone())),
            },

            Add(t1, t2) => Add(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Sub(t1, t2) => Sub(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Mul(t1, t2) => Mul(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),

            Eq(t1, t2) => Eq(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Ne(t1, t2) => Ne(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Lt(t1, t2) => Lt(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Le(t1, t2) => Le(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Gt(t1, t2) => Gt(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Ge(t1, t2) => Ge(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Pair(t1, t2) => Pair(Box::new(t1.subst(x, v.clone())), Box::new(t2.subst(x, v.clone()))),
            Fst(b) => Fst(Box::new(b.as_ref().clone().subst(x, v.clone()))),
            Snd(b) => Snd(Box::new(b.as_ref().clone().subst(x, v.clone()))),

            Nil(ty) => Nil(ty),
            Cons(head, tail) => Cons(
                Box::new(head.subst(x, v.clone())),
                Box::new(tail.subst(x, v.clone())),
            ),
            LCase {
                t,
                nil_t,
                ...
                let t = Box::new(t.subst(x, v.clone()));
                let nil_t = Box::new(nil_t.subst(x, v.clone()));
                if head_var != x && tail_var != x {
                    LCase {
                        t,
                        nil_t,
                        head_var: head_var.clone(),
                        tail_var: tail_var.clone(),
                        cons_t: Box::new(cons_t.subst(x, v)),
                    }
                } else {
                    LCase {
                        t,
            ...
            Case {
                ...
                let t = Box::new(t.subst(x, v.clone()));
                let inl_t = if inl_var != x {
                    Box::new(inl_t.subst(x, v.clone()))
                } else {
                    inl_t
                };
                let inr_t = if inr_var != x {
